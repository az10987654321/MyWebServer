...
/* 将fd上的EPOLLIN和EPOLLET和EPOLLRDHUP事件注册到epollfd指示的epoll内核事件表中，参数oneshot
...
{
    char temp;
    /* m_checked_idx指向buffer（应用程序的读缓冲区）中当前正在分析的字节，m_read_idx
    指向buffer中客户数据的尾部的下一字节。buffer中第0~m_checked_idx字节都已分析完毕，第
    m_checked_idx～(m_read_idx-1)字节由下面的循环挨个分析 */
    for ( ; m_checked_idx < m_read_idx; ++m_checked_idx )
    {
        ...
        if ( temp == '\r' )
        {
            ...
            分析没有读取到一个完整的行，返回LINE_OPEN以表示还需要继续读取客户数据才能
            进一步分析 */
...
读，且不是目录，则使用mmap将其映射到内存地址m_file_address处，并告诉调用者获取文件成功 */
